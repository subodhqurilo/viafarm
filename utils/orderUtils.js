...
function getSpeedPostRate(weightGrams, distanceKm) {
  ...
  const weights = Object.keys(rateTable).map(Number);
  const nearest = weights.find((w) => weight <= w) || 20000;

  return { rate: rateTable[nearest][slab] || 200 };
}
...
async function getDeliveryCharge(buyerId, vendorId, totalWeightKg = 1) {
  try {
    ...
    if (distanceKm <= deliveryRegion) {
      return distanceKm <= 2 ? 50 : 50 + (distanceKm - 2) * 10;
    }
...
async function calculateOrderSummary(cartOrItems, couponOrCode, deliveryType = "delivery") {
  ...
  if (coupon && coupon.minimumOrder && totalMRP < coupon.minimumOrder) coupon = null;
  ...
  const updatedItems = prepared.map((it) => {
    ...
    if (coupon) {
      ...
      const matchProduct =
        coupon.applicableProducts?.some?.(
          (p) => String(p) === String(it.productId)
        );

      if (matchCategory || matchProduct) {
        if (coupon.discount?.type === "Percentage") {
          discount = (it.itemMRP * coupon.discount.value) / 100;
        }
      }
    }
    ...
    return {
      ...it.raw,
      itemMRP: it.itemMRP,
      total: it.itemMRP - discount,
      discount,
    };
  });

  /** --- DELIVERY CHARGE FINAL LOGIC --- */
  ...
  if (deliveryType === "delivery" && vendorId) {
    ...
    deliveryCharge = 0; // pickup ALWAYS zero
  }
